...
class getUserCommand(GeneratingCommand):
    ...
    def generate(self):
        ...
        url = conf['url']
        time_range = '%s>=javascript:gs.daysAgo(%s)' % (daysby, daysAgo) if daysAgo else ''
        ...
        if group_sysid or user_sysid:
            sysparm_query = [time_range, group_sysid, user_sysid]
            sysparm_query = [x for x in sysparm_query if x]
            sysparm_query = '^'.join(sysparm_query)
            query_string = '%s/api/now/table/%s?sysparm_limit=1000&sysparm_query=%s' % (url, 'incident', sysparm_query)
            ...
            for record in getRecords(query_string, {'user': username, 'password':password}):
                ...
                record['opened_at'] = time.mktime(datetime.datetime.strptime(record['sys_created_on'], "%Y-%m-%d %H:%M:%S").timetuple())
                record['resolved_at'] = time.mktime(datetime.datetime.strptime(record['sys_created_on'], "%Y-%m-%d %H:%M:%S").timetuple())
                record['closed_at'] = time.mktime(datetime.datetime.strptime(record['sys_created_on'], "%Y-%m-%d %H:%M:%S").timetuple())
